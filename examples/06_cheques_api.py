...
def main():
    ...
    try:
        ...
        logger.info(f"Checking check status {check_number} for {target_entity.denominacion} (Code: {target_entity.codigo_entidad})...")
        
        # Method A: check_denunciado helper (returns boolean)
        try:
            is_denounced = connector.check_denunciado(target_entity.denominacion, check_number)
            logger.info(f"Check {check_number} denounced status (via check_denunciado): {is_denounced}")
        ...
        try:
             cheque_info = connector.get_cheque_denunciado(target_entity.codigo_entidad, check_number)
             logger.info(f"Cheque info found: {cheque_info}")
        except BCRAApiError as e:
             # It is expected to fail with 404 if the check is not denounced/found
             logger.info(f"get_cheque_denunciado result: {e} (This usually means the check is not denounced)")
