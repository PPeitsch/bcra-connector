...
6. **Report honestly.** If something failed or was skipped, say so. Never claim a
...
| **`.agent/workflows/*` commands fail** | The three commands that use `${CLAUDE_PLUGIN_ROOT}` need the `agent-skills` plugin installed; the rest are plain `git`/`gh`/`pytest`. |
