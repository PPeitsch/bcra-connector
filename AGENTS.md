...
| 8 | **Always merge with `--delete-branch`.** Leaving the branch alive breaks re-targeting of stacked PRs, which then get no CI at all. |
| 9 | **Report failures honestly.** If a test fails or a step was skipped, say so explicitly. Do not claim success you have not verified. |
